...
import { cn, jfetch, toCsv, downloadCsv, currentMonth, formatNumber } from '../../nippo/utils'

export const Route = createFileRoute('/admin/summary')({
  component: SummaryPage,
})

type SummaryRow = { name: string; reports: number; workers: number; area: number }
type SummaryData = {
  bySite: SummaryRow[]
  bySub: SummaryRow[]
...
function SummaryPage() {
  ...
  React.useEffect(() => {
    let cancelled = false
    void (async () => {
      const res = await jfetch<SummaryData>(`/api/admin/summary?month=${month}`)
      if (cancelled) return
