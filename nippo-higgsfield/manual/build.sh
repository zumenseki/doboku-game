...
// 撮影に使う日報URL（現場×業者）を動的に取得する
const asg = await (await d.request.get(\`\${B}/api/admin/assignments?siteId=\${SITE}\`)).json()
const TOKEN = asg.rows?.[0]?.token
if (!TOKEN) throw new Error('この現場に割り当てがありません（先に業者を割り当ててください）')
